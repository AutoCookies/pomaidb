...
{
    if (M == 0) M = 8;
    if (ef_construction == 0) ef_construction = 50;
    if (M > 8) M = 8;
    if (ef_construction > 200) ef_construction = std::min<size_t>(ef_construction, 200);
    if (ef_construction > 50) ef_construction = 50;
    ...
    {
        size_t pick = 16384;
        if (est_ram > pick) pick = est_ram;
        if (est_arena > 0 && est_arena < pick) pick = est_arena;
...
void VectorStore::attach_soa(std::unique_ptr<pomai::ai::soa::VectorStoreSoA> soa)
{
    ...
    else
    {
        ...
        {
            // Ensure dimension matches (if dim_ known)
    ...
    try
    {
        ...
        auto finalize_label_after_insert = [&](uint64_t new_label) -> bool
        {
            if (map_)
            {
                ...
                if (!ok)
                {
                    try { pphnsw_->markDelete(static_cast<hnswlib::labeltype>(new_label)); } catch (...) {}
                    return false;
                }
            }
            {
            ...
            // Prepare fingerprint (if configured) and PQ code (raw or packed) depending on PQ params
            ...
            if (pq_)
            {
                ...
                // Decide storage form:
                // - If PQ has k > 16 we must store raw 8-bit codes (can't pack to 4 bits).
                // - Prefer raw storage only if SoA provides pq_codes block; otherwise fallback to packed4 (with warning).
                ...
                {
                    // store packed4 (works for k <= 16), but also used as fallback if raw not available
                    ...
                    if (need_raw && !soa_has_raw)
                    {
                        std::cerr << "[VectorStore] Warning: PQ k=" << pq_->k()
                                  << " requires raw 8-bit storage but SoA has no pq_codes block; storing packed4 (truncation risk)\n";
                    }
                }
            }
            ...
            if (soa_)
            {
                soa_->append_vector(fp_ptr, fp_len, pq_ptr, pq_len, new_label);
            }
        ...
        if (payload_bytes == dim_ * sizeof(float))
        {
            ...
            else
            {
                ...
                if (!finalize_label_after_insert(new_label)) return false;
                label = new_label;
            }
        }
...
bool VectorStore::remove(const char *key, size_t klen)
{
    ...
    if (label == 0) label = read_label_from_map(key, klen);

    bool map_erased = map_->erase(std::string(key, key + klen).c_str());

    if (label != 0 && pphnsw_)
    {
        try { pphnsw_->markDelete(static_cast<hnswlib::labeltype>(label)); } catch (...) {}
    }
...
std::vector<std::pair<std::string, float>> VectorStore::search(const float *query, size_t dim, size_t topk)
{
    ...
    if (!pphnsw_ || !query || dim != dim_ || topk == 0) return out;
    ...
    if (soa_ && fingerprint_ && soa_->fingerprint_bits() > 0 && soa_->ids_ptr() != nullptr)
    {
        try
        {
            ...
            if (!published_indices.empty())
            {
                ...
                if (hamming_thresh == 0) hamming_thresh = 128;
                ...
                {
                    // map back to SoA indices
                    std::vector<size_t> candidates;
                    candidates.reserve(compact_candidates.size());
                    for (size_t ci : compact_candidates) candidates.push_back(published_indices[ci]);

                    // If PQ trained and SoA has packed4 codes (detect via pq_m/pq_k), use PQ approximate eval to further prune
                    if (pq_ && pq_packed_bytes_ > 0 && soa_->pq_m() != 0 && soa_->pq_k() != 0)
                    {
                        // build packed_compact (contiguous packed4 codes for candidates)
                        ...
                        // compute PQ distance tables
                        std::vector<float> tables;
                        tables.resize(pq_->m() * pq_->k());
                        ...
                        pq_approx_dist_batch_packed4(tables.data(), pq_->m(), pq_->k(), packed_compact.data(), candidates.size(), approx_dists.data());

                        // heuristic: pick top Napprox (min(256, max(topk, 100), candidates.size()))
                        size_t Napprox = std::min<size_t>(std::max<size_t>(topk, 100), 256);
                        Napprox = std::min<size_t>(Napprox, candidates.size());

                        std::vector<size_t> order(candidates.size());
                        for (size_t i = 0; i < order.size(); ++i) order[i] = i;
                        std::nth_element(order.begin(), order.begin() + static_cast<ptrdiff_t>(Napprox), order.end(),
                                         [&](size_t a, size_t b) { return approx_dists[a] < approx_dists[b]; });
                        order.resize(Napprox);
                        std::sort(order.begin(), order.end(), [&](size_t a, size_t b) { return approx_dists[a] < approx_dists[b]; });

                        std::vector<size_t> approx_top;
                        approx_top.reserve(Napprox);
                        for (size_t id : order) approx_top.push_back(candidates[id]);

                        // refine exact L2 on approx_top
                        const uint64_t *ids_block = soa_->ids_ptr();
                        auto refined = pomai::ai::refine::refine_topk_l2(query, dim_, approx_top, ids_block, arena_, topk);
                        ...
                        for (auto &pr : refined)
                        {
                            ...
                            std::string key;
                            {
                                ...
                                if (it != label_to_key_.end()) key = it->second;
                            }
                            if (key.empty() && map_) key = std::to_string(identry);
                    ...
                    {
                        // No PQ: refine directly on candidates
                        const uint64_t *ids_block = soa_->ids_ptr();
    ...
    try
    {
        ...
        while (!pq.empty())
        {
            ...
            if (key.empty() && map_) key = std::to_string(label);
...
{
    if (dim == 0 || max_elements == 0) return false;
    arena_ = arena;
    ...
    if (arena_) pphnsw_->setPomaiArena(arena_);
    ...
    try
    {
        ...
        while (pq_m > 1 && (dim % pq_m) != 0) pq_m /= 2;
        ...
        else
        {
            ...
            for (size_t i = 0; i < n_train_ppq * dim; ++i) samples_ppq[i] = ud(rng);
    ...
    try
    {
        ...
        if (pq_m >= 1 && (dim % pq_m) == 0)
        {
            ...
            for (size_t i = 0; i < n_train * dim; ++i) samples[i] = ud2(rng2);
            ...
            // If SoA mapping is attached and has a codebooks block, persist codebooks into it so the SoA
            // file becomes self-contained (preferred option).
