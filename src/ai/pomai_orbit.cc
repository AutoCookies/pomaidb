...
static constexpr size_t MAX_ECHO_BYTES = 128;

// ----------------- ArenaView dispatch impl -----------------
char *ArenaView::alloc_blob(uint32_t len) const { if (pa) return pa->alloc_blob(len); if (sa) return sa->alloc_blob(len); return nullptr; }
uint64_t ArenaView::offset_from_blob_ptr(const char *p) const noexcept { if (pa) return pa->offset_from_blob_ptr(p); if (sa) return sa->offset_from_blob_ptr(p); return UINT64_MAX; }
const char *ArenaView::blob_ptr_from_offset_for_map(uint64_t offset) const { if (pa) return pa->blob_ptr_from_offset_for_map(offset); if (sa) return sa->blob_ptr_from_offset_for_map(offset); return nullptr; }
std::vector<char> ArenaView::read_remote_blob(uint64_t remote_id) const { if (pa) return pa->read_remote_blob(remote_id); if (sa) return sa->read_remote_blob(remote_id); return {}; }
...
uint64_t PomaiOrbit::alloc_new_bucket(uint32_t centroid_id)
{
    ...
    for (size_t i = 0; i < cap; ++i) len_base[i] = 0;
...
bool PomaiOrbit::insert(const float *vec, uint64_t label)
{
    ...
    for (uint8_t q : code.scales_q) buf.push_back(q);
    for (const auto &sb : code.sign_bytes) buf.insert(buf.end(), sb.begin(), sb.end());

    if (buf.size() > MAX_ECHO_BYTES) return false; // too large
...
bool PomaiOrbit::get(uint64_t label, std::vector<float> &out_vec)
{
    ...
    else data_base_ptr = ram_blob_ptr + sizeof(uint32_t);
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        if (v == label) { found = static_cast<int32_t>(i); break; }
    }
    ...
    for (size_t k = 0; k < depth; ++k) { if (pos < len) code.scales_q[k] = ub[pos++]; else code.scales_q[k] = 0; }
    ...
    const float *query, size_t k, const std::vector<uint64_t> &candidates, const pomai::ai::Budget &budget)
{
    ...
    {
        std::shared_lock<std::shared_mutex> dm(del_mu_);
        if (!deleted_labels_.empty()) local_deleted = deleted_labels_;
    }

    for (uint64_t id : candidates)
    {
        if (ops_left == 0) break;
        if (!local_deleted.empty() && local_deleted.count(id)) continue;
        ...
        uint64_t bucket_off = 0;
        {
            ...
            if (it == label_to_bucket_.end()) continue;
            bucket_off = it->second;
        }
        if (bucket_off == 0) continue;
    ...
    for (uint32_t cid : targets)
    {
        ...
        while (current_off != 0)
        {
            ...
            for (uint32_t i = 0; i < count; ++i)
            {
                ...
                size_t depth = code.depth;
                code.scales_q.resize(depth);
                for (size_t kk = 0; kk < depth; ++kk) { if (pos < len) code.scales_q[kk] = ub[pos++]; else code.scales_q[kk] = 0; }
                code.bits_per_layer.resize(depth);
                code.sign_bytes.resize(depth);
                bool bad = false;
    ...
    const float *query, size_t k, const std::vector<uint64_t> &candidates)
{
    ...
    while (idx < entries.size())
    {
        ...
        if (!ram_blob_ptr)
        {
            ...
            if (temp_buf.empty())
            {
                while (idx < entries.size() && entries[idx].first == bucket_off) ++idx;
                continue;
            }
        ...
        const BucketHeader *hdr_ptr = reinterpret_cast<const BucketHeader *>(data_base_ptr);
        uint32_t count = hdr_ptr->count.load(std::memory_order_acquire);
        if (count == 0) { while (idx < entries.size() && entries[idx].first == bucket_off) ++idx; continue; }
        ...
        while (idx < entries.size() && entries[idx].first == bucket_off)
        {
            ...
            size_t depth = code.depth;
            code.scales_q.resize(depth);
            for (size_t k2 = 0; k2 < depth; ++k2) { if (pos < len) code.scales_q[k2] = ub[pos++]; else code.scales_q[k2] = 0; }
            code.bits_per_layer.resize(depth);
            code.sign_bytes.resize(depth);
            bool bad = false;
            for (size_t k2 = 0; k2 < depth; ++k2)
            {
                ...
                size_t bytes = (b + 7) / 8;
                if (pos + bytes <= len) { code.sign_bytes[k2].assign(ub + pos, ub + pos + bytes); pos += bytes; }
                else { bad = true; break; }
            }
            if (bad) continue;
...
bool PomaiOrbit::compute_distance_for_id(const float *query, uint64_t id, float &out_dist)
{
    ...
    {
        uint64_t v = pomai::ai::atomic_utils::atomic_load_u64(id_base + i);
        if (v == id) { found = static_cast<int32_t>(i); break; }
    }
    if (found < 0) return false;
    ...
    for (size_t k = 0; k < depth; ++k)
    {
        ...
        code.bits_per_layer[k] = b;
        size_t bytes = (b + 7) / 8;
        if (pos + bytes <= len) { code.sign_bytes[k].assign(ub + pos, ub + pos + bytes); pos += bytes; }
        else return false;
    }
