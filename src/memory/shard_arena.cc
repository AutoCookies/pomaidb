...
{
    if (!ptr || !base_addr_) return UINT64_MAX;
    intptr_t diff = reinterpret_cast<const char *>(ptr) - base_addr_;
    if (diff <= 0 || static_cast<size_t>(diff) >= capacity_) return UINT64_MAX;
...
const char *ShardArena::blob_ptr_from_offset_for_map(uint64_t offset) const noexcept
{
    ...
    if (is_encoded_remote_id(offset)) {
        ...
        if (it != remote_mmaps_.end()) return it->second.first;
    ...
    if (offset >= wh) return nullptr; // not yet published
...
{
    if (!base_addr_) return 0;
    if (local_offset == 0 || local_offset >= capacity_) return 0;
...
{
    if (!is_encoded_remote_id(remote_id)) return {};
    ...
    if (!ifs.read(buf.data(), sz)) return {};
    return buf;
}
...
void ShardArena::reset() noexcept
{
    ...
    for (auto &ent : remote_mmaps_) {
        const char *addr = ent.second.first;
        size_t sz = ent.second.second;
        if (addr && sz > 0) munmap(const_cast<char *>(addr), sz);
    }
    remote_mmaps_.clear();
