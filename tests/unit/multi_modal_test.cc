...
POMAI_TEST(MultiModal_AutoEdgeHookCreatesSemanticLink) {
    ...
    // 3. Verify that an edge (100 -> 500) was created automatically
    std::vector<Neighbor> nb;
    POMAI_EXPECT_OK(db.GetNeighbors(100, &nb));
    
    bool found = false;
    for (const auto& n : nb) {
        if (n.id == static_cast<VertexId>(vid)) {
            found = true;
            break;
        }
    }
...
POMAI_TEST(MultiModal_AtomicSyncConsistencyCheck) {
    ...
    std::vector<Neighbor> nb;
    POMAI_EXPECT_OK(db.GetNeighbors(200, &nb));
