...
// #define PALLOC_GUARDED
// #define USE_STD_MALLOC

// > palloc-test-stress [THREADS] [SCALE] [ITER]
//
// argument defaults
#if defined(PALLOC_TSAN)          // with thread-sanitizer reduce the threads to test within the azure pipeline limits
static int THREADS = 8;
static int SCALE   = 25;
static int ITER    = 400;
#elif defined(PALLOC_UBSAN)       // with undefined behavious sanitizer reduce parameters to stay within the azure pipeline limits
static int THREADS = 8;
static int SCALE   = 25;
static int ITER    = 20;
#elif defined(PALLOC_GUARDED)     // with debug guard pages reduce parameters to stay within the azure pipeline limits
static int THREADS = 8;
static int SCALE   = 10;
static int ITER    = 10;
#else
...
#define custom_calloc(n,s)    palloc_calloc(n,s)
#define custom_realloc(p,s)   palloc_realloc(p,s)
#define custom_free(p)        palloc_free(p)
...
#ifdef HEAP_WALK
static bool visit_blocks(const palloc_heap_t* heap, const palloc_heap_area_t* area, void* block, size_t block_size, void* arg) {
  (void)(heap); (void)(area);
...
static void stress(intptr_t tid) {
  ...
  // walk the heap
  size_t total = 0;
  palloc_heap_visit_blocks(palloc_heap_get_default(), true, visit_blocks, &total);
  #endif
...
static void test_stress(void) {
  ...
  for (int n = 0; n < ITER; n++) {
    ...
    // palloc_option_set_enabled(palloc_option_disallow_arena_alloc, (n%2)==1);
    #endif
    #ifdef HEAP_WALK
    size_t total = 0;
    palloc_abandoned_visit_blocks(palloc_subproc_main(), -1, true, visit_blocks, &total);
    #endif
    ...
    //palloc_debug_show_arenas(true);
    #endif
    #if !defined(NDEBUG) || defined(PALLOC_TSAN)
...
#if defined(USE_STD_MALLOC) && defined(PALLOC_LINK_VERSION)
#ifdef __cplusplus
extern "C"
#endif
int palloc_version(void);
#endif

int main(int argc, char** argv) {
  #ifdef PALLOC_LINK_VERSION
    palloc_version();
  #endif
  #ifdef HEAP_WALK
    palloc_option_enable(palloc_option_visit_abandoned);
  #endif
  #if !defined(NDEBUG) && !defined(USE_STD_MALLOC)
    palloc_option_set(palloc_option_arena_reserve, 32 * 1024 /* in kib = 32MiB */);
  #endif
  ...
  //palloc_reserve_os_memory(1024*1024*1024ULL, false, true);
  //int res = palloc_reserve_huge_os_pages(4,1);
  ...
  //palloc_reserve_os_memory(512ULL << 20, true, true);
  ...
  palloc_collect(true);
  char* json = palloc_stats_get_json(0, NULL);
  if (json != NULL) {
    fputs(json,stderr);
    palloc_free(json);
  }
