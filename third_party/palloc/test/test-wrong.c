...
/* test file for valgrind/asan support.
   ...
   > gcc -g -o test-wrong -I../../include ../../test/test-wrong.c libpalloc-valgrind-debug.a -lpthread
   ...
   > clang -g -o test-wrong -I../../include ../../test/test-wrong.c libpalloc-asan-debug.a -lpthread -fsanitize=address -fsanitize-recover=address
...
# define mi(x) x
#else
# define mi(x) palloc_##x
#endif

int main(int argc, char** argv) {
  int* p = (int*)mi(malloc)(3*sizeof(int));

  int* r = (int*)palloc_malloc_aligned(8,16);
  palloc_free(r);
