...
int main() {
  palloc_stats_reset();  // ignore earlier allocations
  // various_tests();
  // test_mixed1();
  // test_dep();
...
static void various_tests() {
  atexit(free_p);
  void* p1 = malloc(78);
  void* p2 = palloc_malloc_aligned(24, 16);
  free(p1);
  p1 = malloc(8);
  char* s = palloc_strdup("hello\n");
...
#if PALLOC_HAS_HEAP_STL_ALLOCATOR
static bool test_stl_allocator3() {
  std::vector<int, palloc_heap_stl_allocator<int> > vec;
  vec.push_back(1);
  vec.pop_back();
  return vec.size() == 0;
}
...
static bool test_stl_allocator5() {
  std::vector<int, palloc_heap_destroy_stl_allocator<int> > vec;
  vec.push_back(1);
  vec.pop_back();
  return vec.size() == 0;
}
...
{
  cptr = palloc_calloc_tp(char,22085632);
  return NULL;
}
...
static void strdup_test() {
  ...
  char* buf = NULL;
  size_t len;
  _dupenv_s(&buf, &len, "PALLOC_VERBOSE");
  palloc_free(buf);
  palloc_free(s);
#endif
}
...
static void heap_no_delete_worker() {
  ...
  void* q = palloc_heap_malloc(heap, 1024); (void)(q);
  // palloc_heap_delete(heap); // uncomment to prevent assertion
}
...
static void t1main() {
  palloc_heap_t* heap = palloc_heap_new();
  global_p = palloc_heap_malloc(heap, 1024);
...
static void heap_thread_free_large() {
  for (int i = 0; i < 100; i++) {
    shared_p = palloc_malloc_aligned(2 * 1024 * 1024 + 1, 8);
...
static void heap_thread_free_huge() {
  for (int i = 0; i < 100; i++) {
    shared_p = palloc_malloc(1024 * 1024 * 1024);
...
// https://github.com/microsoft/palloc/issues/1104
...
static void test_perf_local_alloc()
{
  ...
  {
    int len = LEN[rng() % 4];
    int* p = (int*)palloc_zalloc_aligned(len * sizeof(int), alignof(int));
    p[0] = 1;
    sum += p[rng() % len];
    free(p);
  }
...
static void local_alloc5() {
  ...
  for (int i = 0; i < 500000; i++) {
    const size_t n = i % 1000;
    uint8_t* p = (uint8_t*)palloc_malloc(n);
    escape5(p, n);
    if (i % 4 > 0) {
      if (n>0) { sum += p[n-1]; }
      palloc_free(p);
