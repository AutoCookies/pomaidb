...
#include "palloc/prim.h"    // _palloc_prim_random_buf
...
#define PALLOC_CHACHA_ROUNDS (20)   // perhaps use 12 for better performance?
...
static void chacha_block(palloc_random_ctx_t* ctx)
{
  // scramble into `x`
  uint32_t x[16];
  ...
  for (size_t i = 0; i < PALLOC_CHACHA_ROUNDS; i += 2) {
    qround(x, 0, 4,  8, 12);
    qround(x, 1, 5,  9, 13);
    qround(x, 2, 6, 10, 14);
    qround(x, 3, 7, 11, 15);
...
static uint32_t chacha_next32(palloc_random_ctx_t* ctx) {
...
static void chacha_init(palloc_random_ctx_t* ctx, const uint8_t key[32], uint64_t nonce)
{
...
static void chacha_split(palloc_random_ctx_t* ctx, uint64_t nonce, palloc_random_ctx_t* ctx_new) {
  memset(ctx_new, 0, sizeof(*ctx_new));
  _palloc_memcpy(ctx_new->input, ctx->input, sizeof(ctx_new->input));
  ctx_new->input[12] = 0;
  ctx_new->input[13] = 0;
  ctx_new->input[14] = (uint32_t)nonce;
  ctx_new->input[15] = (uint32_t)(nonce >> 32);
  palloc_assert_internal(ctx->input[14] != ctx_new->input[14] || ctx->input[15] != ctx_new->input[15]); // do not reuse nonces!
...
#if PALLOC_DEBUG>1
static bool palloc_random_is_initialized(palloc_random_ctx_t* ctx) {
  return (ctx != NULL && ctx->input[0] != 0);
}
#endif

void _palloc_random_split(palloc_random_ctx_t* ctx, palloc_random_ctx_t* ctx_new) {
  palloc_assert_internal(palloc_random_is_initialized(ctx));
  palloc_assert_internal(ctx != ctx_new);
...
uintptr_t _palloc_random_next(palloc_random_ctx_t* ctx) {
  ...
  do {
    ...
    r = chacha_next32(ctx);
    #elif PALLOC_INTPTR_SIZE == 8
    r = (((uintptr_t)chacha_next32(ctx) << 32) | chacha_next32(ctx));
    #else
    # error "define palloc_random_next for this platform"
    #endif
  } while (r==0);
  return r;
}
...
uintptr_t _palloc_os_random_weak(uintptr_t extra_seed) {
  uintptr_t x = (uintptr_t)&_palloc_os_random_weak ^ extra_seed; // ASLR makes the address random
  x ^= _palloc_prim_clock_now();  
  // and do a few randomization steps
  uintptr_t max = ((x ^ (x >> 17)) & 0x0F) + 1;
  for (uintptr_t i = 0; i < max || x==0; i++, x++) {
    x = _palloc_random_shuffle(x);
  }
  palloc_assert_internal(x != 0);
  return x;
}

static void palloc_random_init_ex(palloc_random_ctx_t* ctx, bool use_weak) {
  uint8_t key[32];
  if (use_weak || !_palloc_prim_random_buf(key, sizeof(key))) {
    // if we fail to get random data from the OS, we fall back to a
    // weak random source based on the current time
    #if !defined(__wasi__)
    if (!use_weak) { _palloc_warning_message("unable to use secure randomness\n"); }
    #endif
    uintptr_t x = _palloc_os_random_weak(0);
...
static void chacha_test(void)
{
  ...
  qround(x, 0, 1, 2, 3);
  palloc_assert_internal(array_equals(x, x_out, 4));
  ...
  qround(y, 2, 7, 8, 13);
  palloc_assert_internal(array_equals(y, y_out, 16));

  palloc_random_ctx_t r = {
  ...
  chacha_block(&r);
  palloc_assert_internal(array_equals(r.output, r_out, 16));
}
*/
